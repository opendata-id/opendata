...
class DataService {
    ...
    private function getLatestYear(): int {
        $result = $this->query("SELECT MAX(year) as year FROM wages", 'latest_year');
        return ($result && count($result) > 0 && $result[0]['year']) ? (int)$result[0]['year'] : (int)date('Y');
    }
    ...
    public function getEconomicIndicators(): array {
        ...
        $highest = $this->query("
            ...
            WHERE w.year = {$year} ORDER BY w.umr DESC LIMIT 1
        ", "umr_highest_{$year}");
        ...
        $lowest = $this->query("
            SELECT r.name, r.province, w.umr
            FROM wages w JOIN regions r ON w.region_id = r.id
            WHERE w.year = {$year} ORDER BY w.umr ASC LIMIT 1
        ", "umr_lowest_{$year}");
    ...
    public function getProjects(): array {
        ...
        return [
            [
                'name' => 'Dashboard Data',
                'slug' => 'data',
                'description' => "Jelajahi data UMR {$year}, harga pangan, dan inflasi. Filter, cari, dan analisis indikator ekonomi seluruh wilayah Indonesia.",
                'status' => 'active',
                'regions' => $regionCount,
