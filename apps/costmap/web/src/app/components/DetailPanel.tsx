...
import type { RegionProperties } from './Map';
...
export const DetailPanel = memo(function DetailPanel({ region, onClose }: DetailPanelProps) {
  ...
  const isUsingUmp = !isProvince && region.umr && region.ump && region.umr === region.ump;
        ...
        >
          <div className="flex items-baseline justify-between">
            <span className="text-[10px] uppercase tracking-[0.15em] text-muted font-medium">
              {isProvince
                ? `UMP ${new Date().getFullYear()}`
                : isUsingUmp
                  ? `UMP ${new Date().getFullYear()}`
                  : `UMK ${new Date().getFullYear()}`
              }
            </span>
          ...
          {isUsingUmp && (
            <div
              className={`
                mt-2 py-1.5 px-2 bg-amber-50 border border-amber-200 text-amber-700
            ...
            >
              <span className="text-[11px]">
                Provinsi ini tidak menetapkan UMK
              </span>
            </div>
          )}
        </div>
